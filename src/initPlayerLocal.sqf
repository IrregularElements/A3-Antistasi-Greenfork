...
if (isMultiplayer) then
	...
	grupo setVariable ["origen",respawnBuenos];
...
player addEventHandler ["WeaponAssembled",
	...
	if (_veh isKindOf "StaticWeapon") then
		...
		if !(_veh getVariable ["staticsToSave",false]) then
			{
			_veh setVariable ["staticsToSave",true,true];
...
player addEventHandler ["WeaponDisassembled",
		...
		_pos = getPosATL _bag1;
		if (_pos select 2 > 0) then
			{
			_tipo = typeOf _bag1;
			_pos = player getPos [3,(getDir player) + 10];
			_bag = _tipo createVehicle _pos;
			[_bag] call A3A_fnc_AIVEHinit;
			deleteVehicle _bag1;
			};
		_pos = getPosATL _bag2;
		if (_pos select 2 > 0) then
			{
			_tipo = typeOf _bag2;
			_pos = player getPos [3,(getDir player) - 10];
