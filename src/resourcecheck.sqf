...
while {true} do
	...
	if (_ciudad in destroyedCities) then
		...
		_hrAddCiudad = (_numciv * (_prestigeSDK / 15000));///20000 originalmente
		switch (_power) do
			{
	...
	if ((!bigAttackInProgress) and (random 100 < 50) and (tierWar <= 5)) then {[] call A3A_fnc_missionRequestAUTO};
