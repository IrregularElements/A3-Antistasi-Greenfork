...
_noAmmo = ({_x in _magazines} count (magazines _unit) == 0);
if ((_Pweapon in initialRifles) or (_Pweapon == "") or _noAmmo) then
	{
	_necesita = true;
		...
		if (_unit distance _objeto < _distancia) then
			...
			if ((count weaponCargo _objeto > 0) and !(_objeto getVariable ["busy",false])) then
				...
				for "_i" from 0 to (count _armas - 1) do
					...
					if (((not(_basePosible in initialRifles)) and (_basePosible in (arifles+srifles+mguns))) or _noAmmo) then
						{
						_target = _objeto;
						_hayCaja = true;
...
_hayCaja = false;
_cuenta = 4;
if (_Pweapon in mguns) then {_cuenta = 3};
_magazines = getArray (configFile / "CfgWeapons" / _Pweapon / "magazines");
