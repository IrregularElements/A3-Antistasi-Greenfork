...
if (_enemigo isEqualType objNull) then
	{
	_enemigo = position _enemigo
	}
else
	{
	if (_enemigo isEqualType "") then
		{
		_enemigo = getMarkerPos _enemigo;
		}
	};
_small= [];
_big = [];
...
if ((_big isEqualTo []) and (_small isEqualTo [])) exitWith {[]};
