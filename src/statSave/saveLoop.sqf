...
if (!isDedicated) then
 ...
 private ["_garrison"];
	...
	["securedCities", securedCities] call fn_SaveStat;
...
_veh = _x;
_tipoVeh = typeOf _veh;
if ((_veh distance getMarkerPos respawnBuenos < 50) and !(_veh getVariable ["staticsToSave",false]) and !(_tipoVeh in ["ACE_SandbagObject","Land_PaperBox_01_open_boxes_F","Land_PaperBox_01_open_empty_F"])) then
	{
...
} forEach (vehicles select {_x getVariable ["staticsToSave",false]});
...
{
_posMina = getPos _x;
if !(surfaceIsWater _posMina) then
	{
	_tipoMina = typeOf _x;
	_dirMina = getDir _x;
	_detectada = [];
