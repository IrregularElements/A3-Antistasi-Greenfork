...
while {alive _veh} do
	{
	if (_veh getVariable ["staticsToSave",false]) exitWith {};
	if ((!([distanciaSPWN,1,_veh,[buenos,malos,muyMalos]] call A3A_fnc_distanceUnits)) and {_veh distance getMarkerPos respawnBuenos > 100}) then
		{
		deleteVehicle _veh
		};
	sleep 60;
