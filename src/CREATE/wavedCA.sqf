...
if (_mrkDestino in ciudades) then
	{
	[_ladosTsk,"AtaqueAAF",[format ["%2 Is attacking %1 in order to regain support in the city. Defend it to secure the city",_nombreorig,_nombreDest],format ["The Battle of %1",_nombreDest],_mrkDestino],getMarkerPos _mrkDestino,false,0,true,"Defend",true] call BIS_fnc_taskCreate;
	}
else
	{
...
while {(_waves > 0)} do
		...
		while {(_cuenta <= _nVeh) and (count _soldados <= 80)} do
			...
			if (_proceder) then
				...
				if (not(_tipoVeh in vehTanks)) then
					...
					_veh allowCrewInImmobile true;
					_grupoVeh spawn A3A_fnc_globalInfoShare;
					};
				};
				sleep 15;
				_cuenta = _cuenta + 1;
		...
		if !(_pos isEqualTo []) then
			...
			while {(_cuenta < 3) and (count _soldados <= 80)} do
				...
				if !(_landPos isEqualTo []) then
					{
					_newPos = _landPos;
					_ang = _landPos getDir _posDestino;
					while {true} do
						{
						_newPos = _landPos getPos [10,_ang];
						if !(surfaceIsWater _newPos) exitWith {};
						_landPos = _newPos;
						};
					};
				if (!(_landPos isEqualTo []) and _proceder) then
					{
					_pos = _pos findEmptyPosition [0,50,_tipoVeh];
					...
					[_veh] call A3A_fnc_AIVEHinit;
					_veh setVelocityModelSpace [((velocityModelSpace _veh) select 0) + 0,((velocityModelSpace _veh) select 1) + 5,((velocityModelSpace _veh) select 2)];
					if ((_tipoVeh == vehNATOBoat) or (_tipoVeh == vehCSATBoat)) then
						{
						...
						if (_tipoVeh in vehAPCs) then
							...
							_Vwp2 setWaypointStatements ["true", "{[_x] orderGetIn false; unassignVehicle _x} forEach (units group this);(group this) spawn A3A_fnc_attackDrillAI"];
							...
							_Vwp setWaypointStatements ["true", "{[_x] orderGetIn false; unassignVehicle _x} forEach (units group this);(group this) spawn A3A_fnc_attackDrillAI"];
							_Vwp1 = _grupoVeh addWaypoint [_posDestino, 1];
							_Vwp1 setWaypointType "SAD";
							_Vwp1 setWaypointBehaviour "COMBAT";
							//[_veh,"Boat"] spawn A3A_fnc_inmuneConvoy;
						...
						_grupo spawn
							...
							if (surfaceIsWater (position _lider)) then
								{
								if (_lider != vehicle _lider) then
									{
									private _veh = vehicle _lider;
	...
	if ([_mrkDestino,true] call A3A_fnc_fogCheck >= 0.3) then
		...
		[_mrkOrigen,15] call A3A_fnc_addTimeForIdle;
		...
		if !(hayIFA) then
			...
			_grupoUav spawn A3A_fnc_globalInfoShare;
	...
	if (_lado == malos) then
		...
		if (((not(_mrkDestino in puestos)) and (not(_mrkDestino in puertos)) and (_mrkOrigen != "NATO_carrier")) or hayIFA) then
			{
			if (_artyData isEqualTo []) then {_artyData = [_mrkOrigen,_mrkDestino,_lado] call A3A_fnc_artilleria};
	...
	if !(_artyData isEqualto []) then
		{
		_vehiculos pushBack (_artyData select 0);
		_pilotos append (_artyData select 1);
		{if (isNull (_x getVariable ["artySupport",objNull])) then {_x setVariable ["artySupport",(_artyData select 0)]}} forEach _grupos;
		_grupos pushBack (_artyData select 2);
		};
	if ((!_SDKShown) and (!(_mrkDestino in ciudades))) then
		{
	...
	_waves = _waves -1;
	_firstWave = false;
	{_x setVariable ["origen",_mrkOrigen]} forEach _grupos;
	...
	if (_lado == malos) then
		...
		if  ((({[_x,_mrkDestino] call A3A_fnc_canConquer} count _soldados) > 3*({(side _x != _lado) and (side _x != civilian) and ([_x,_mrkDestino] call A3A_fnc_canConquer)} count allUnits)) or (lados getVariable [_mrkDestino,sideUnknown] == malos)) then
			...
			if (_mrkDestino in ciudades) then
				...
				["AtaqueAAF",[format ["%2 Is attacking %1 in order to regain support in the city. Defend it to secure the city",_nombreorig,_nombreDest],format ["The Battle of %1",_nombreDest],_mrkDestino],getMarkerPos _mrkDestino,"FAILED"] call A3A_fnc_taskUpdate;
				...
				if (!(lados getVariable [_mrkDestino,sideUnknown] == malos)) then {[malos,_mrkDestino] remoteExec ["A3A_fnc_markerChange",2]};
				};
			};
		sleep 10;
		if (!(lados getVariable [_mrkDestino,sideUnknown] == malos)) then
			...
			if ((_waves <= 0) or (!(lados getVariable [_mrkOrigen,sideUnknown] == malos)) or (([distanciaSPWN/2,1,_posOrigen,buenos] call A3A_fnc_distanceUnits) and (({_x distance _veh <= distanciaSPWN/2} count ((allPlayers select {side group _x != _lado}) - (entities "HeadlessClient_F"))) == 0))) then
				{
				{_x doMove _posorigen} forEach _soldadosTotal;
				if (_waves <= 0) then
					{
					[_mrkDestino,_mrkOrigen] call A3A_fnc_minefieldAAF
					}
				else
					{
					_waves = 0;
				...
				if (_mrkDestino in ciudades) then
					{
					["AtaqueAAF",[format ["%2 Is attacking %1 in order to regain support in the city. Defend it to secure the city",_nombreorig,_nombreDest],format ["The Battle of %1",_nombreDest],_mrkDestino],getMarkerPos _mrkDestino,"SUCCEEDED"] call A3A_fnc_taskUpdate;
					securedCities pushBack _mrkDestino;
					publicVariable "securedCities";
					}
				else
					{
		...
		if (!(lados getVariable [_mrkDestino,sideUnknown] == muyMalos)) then
			...
			if ((_waves <= 0) or (lados getVariable [_mrkOrigen,sideUnknown] != muyMalos) or (([distanciaSPWN/2,1,_posOrigen,buenos] call A3A_fnc_distanceUnits) and (({_x distance _veh <= distanciaSPWN/2} count ((allPlayers select {side group _x != _lado}) - (entities "HeadlessClient_F"))) == 0))) then
				{
				{_x doMove _posorigen} forEach _soldadosTotal;
				if (_waves <= 0) then
					{
