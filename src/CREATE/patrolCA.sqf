...
if (_base != "") then
	...
	{_x setVariable ["origen",_base]} forEach _grupos;
	...
	for "_i" from 1 to _cuenta do
		...
		if (not (_tipoVeh in vehTransportAir)) then
			...
			if (_proceder) then
				{
				_dist = 400;
				_ang = ((_posDestino getDir _posOrigen) + (random 90) - 45);
				_landPos = [];
				while {_dist < 800} do
					{
					_provPos = _posDestino getPos [_dist,_ang];
					if (_enemigos findIf {_x distance _provPos < 300}==-1) exitWith {_landPos = _provPos};
					_dist = _dist + 50;
					};
				//_landPos = [_posDestino, 300, 550, 10, 0, 0.20, 0,[],[[0,0,0],[0,0,0]]] call BIS_fnc_findSafePos;
				if !(_landPos isEqualTo []) then {_landPos = [_landPos, 0, 150, 10, 0, 0.20, 0,[],[[0,0,0],[0,0,0]]] call BIS_fnc_findSafePos};
	...
	{_x setVariable ["origen",_aeropuerto]} forEach _grupos;
